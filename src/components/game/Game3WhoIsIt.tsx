...
export default function Game3WhoIsIt({ roomId, players, isHost }: GameProps) {
  ...
  const [votes, setVotes] = useState<Record<string, string>>({}); // voterId -> votedId
  ...
  // If host and first time, pick a question
  useEffect(() => {
  ...
  // Host auto-end voting if everyone voted
  useEffect(() => {
  ...
  const castVote = async (playerId: string) => {
    ...
    await fetch(`/api/rooms/${roomId}/game/action`, {
      ...
      body: JSON.stringify({ 
        event: 'whoisit-vote', 
        data: { voterId: myId, votedId: playerId } 
      }),
    });
  };
  ...
  if (phase === 'result') {
    ...
    return (
      <Card className="p-8 text-center max-w-2xl mx-auto w-full">
              ...
              losers.map(loserId => {
                ...
                return (
                  <div key={loserId} className="flex flex-col items-center">
                    ...
                    <span className="text-sm text-red-500 mt-1">({maxVotes} phiếu)</span>
                  </div>
                );
              })
            ) : (
        ...
        <p className="text-lg font-bold mb-8">🍻 {losers.length > 0 ? 'Những người trên phải uống 1 ly!' : 'Không ai phải uống!'}</p>
